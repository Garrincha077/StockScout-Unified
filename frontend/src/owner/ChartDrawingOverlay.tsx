...
export default function ChartDrawingOverlay({chart,candle,bars,ticker,interval}:{chart:IChartApi;candle:ISeriesApi<'Candlestick'>;bars:ChartBar[];ticker:string;interval:string}){
  ...
  const commit=useCallback((type:Exclude<DrawingTool,'cursor'>,start:DrawingPoint,end:DrawingPoint)=>{
    ...
    const label=type==='text'?window.prompt('Text label:',ticker)??ticker:undefined
    void save(newDrawing(type,points,label));setDraft(null);setCursor(null);setTool('cursor');setPanLocked(true)
  },[save,ticker])
  ...
  const savePriceAlert=()=>{
    const level=Number(window.prompt('Price alert level:',String(bars.at(-1)?.close??'')))
    if(!Number.isFinite(level)||level<=0)return
    setBusy(true);setMessage('')
    owner.saveAlert({name:`${ticker} crossing ${level.toFixed(2)}`,ticker,enabled:true,payload:{kind:'price',operator:'crossing',price:level}})
      .then(()=>setMessage('Price alert synced.')).catch(error=>setMessage(error instanceof Error?error.message:String(error))).finally(()=>setBusy(false))
  }
  const saveDrawingAlert=()=>{
    if(!selected)return
    const operator=['rectangle','channel'].includes(selected.payload.type)?'entering':'crossing'
    setBusy(true);setMessage('')
    owner.saveAlert({name:`${ticker} ${selected.payload.type} ${operator}`,ticker,enabled:true,payload:{kind:'drawing',operator,drawing:selected.payload}})
      .then(()=>setMessage('Drawing alert synced.')).catch(error=>setMessage(error instanceof Error?error.message:String(error))).finally(()=>setBusy(false))
  }

  return<>
    <div className="chart-drawing-toolbar" aria-label="Chart drawing tools">
      {!owner.user?<span>{owner.configured?'Sign in to draw':'Owner drawing sync not configured'}</span>:<>
